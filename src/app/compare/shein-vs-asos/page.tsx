...
export default function BitdefenderVsNorton() {
  ...
  const faqJsonLd = {
    ...
    '@type': 'FAQPage',
    mainEntity: [
      { '@type': 'Question', name: 'Does Bitdefender slow down your computer?', acceptedAnswer: { '@type': 'Answer', text: 'No. Bitdefender offloads most scanning to the cloud and uses a minimal local footprint. In AV-TEST performance benchmarks, it consistently ranks among the lowest-impact antiviruses.' } },
      { '@type': 'Question', name: 'Is Norton worth the price?', acceptedAnswer: { '@type': 'Answer', text: 'Depends on the plan. Base Norton 360 is competitive. The higher LifeLock plans are worth it if you need identity theft monitoring -- they get pricey but include up to $1M in coverage.' } },
      { '@type': 'Question', name: 'Which antivirus is better for Mac?', acceptedAnswer: { '@type': 'Answer', text: 'Both have solid Mac versions. Bitdefender has historically optimized better for macOS, while Norton\'s Mac version is more feature-complete than it used to be.' } },
    ],
  };
  ...
  return (
    <article className="pt-8 max-w-3xl mx-auto">
      ...
      <div className="border border-gray-700 rounded-xl p-6 mb-8 bg-gray-800/50">
        ...
        <p className="text-sm text-gray-400 mt-4 text-center"><strong className="text-white">Our pick:</strong> Bitdefender for pure antivirus. Norton if you want identity theft coverage bundled in.</p>
      </div>
          ...
          <tbody>
            {data.map((r, i) => (
              <tr key={i} className="border-t border-gray-800">
                ...
                <td className={`px-4 py-3 text-sm text-center ${r.winner === 'bitdefender' ? 'text-emerald-400 font-semibold' : 'text-gray-400'}`}>{r.bitdefender}</td>
                <td className={`px-4 py-3 text-sm text-center ${r.winner === 'norton' ? 'text-emerald-400 font-semibold' : 'text-gray-400'}`}>{r.norton}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="space-y-6 text-gray-300">
        <div>
          <h2 className="font-body text-xl font-bold text-white mb-3">Detection: Bitdefender Has a Consistent Lead</h2>
          <p>AV-TEST and AV-Comparatives both give Bitdefender top marks, consistently hitting 99.9% malware detection with near-zero false positives. Norton scores well too -- around 99.6% -- but occasionally flags legitimate software. For most users that gap is negligible. For business endpoint protection, every missed detection matters.</p>
          <p className="mt-3">Bitdefender's ransomware remediation layer is worth highlighting. If ransomware somehow slips through, Bitdefender can roll back encrypted files automatically. Norton does not have an equivalent feature at most price tiers.</p>
        </div>

        <div>
          <h2 className="font-body text-xl font-bold text-white mb-3">System Impact: A Noticeable Difference in Daily Use</h2>
          <p>Bitdefender runs almost invisibly -- scans happen in the cloud where possible, and local CPU usage stays low even during full-disk scans. Norton's impact is more noticeable, especially during initial setup and scheduled scans. On older hardware, that difference matters a lot.</p>
        </div>

        <div>
          <h2 className="font-body text-xl font-bold text-white mb-3">Identity Protection: Norton's Clear Advantage</h2>
          <p>If you specifically want identity theft protection, Norton is the stronger pick. Their LifeLock-powered plans include credit monitoring, SSN alerts, and up to $1 million in identity theft insurance on higher-tier plans. Bitdefender offers basic identity protection features but nothing that matches LifeLock's depth.</p>
          <p className="mt-3">Norton also includes a genuinely unlimited VPN on their higher plans. Bitdefender caps its bundled VPN at 200MB per day on base plans, which runs out fast if you use it regularly.</p>
        </div>
        ...
        <div>
          ...
          <p className="mt-2"><strong className="text-white">Get Norton</strong> if you want identity theft coverage and an unlimited VPN bundled in. The 360 plans offer solid all-in-one value if you would otherwise be paying for these tools separately.</p>
        </div>

        <div>
          <h2 className="font-body text-xl font-bold text-white mb-3">FAQ</h2>
          {[
            { q: 'Does Bitdefender slow down your computer?', a: 'Not meaningfully. Bitdefender offloads most scanning to the cloud. In benchmark tests, it consistently ranks among the lowest-impact antiviruses available.' },
            ...
            { q: 'Which works better on Mac?', a: 'Both have solid Mac versions. Bitdefender has historically optimized better for macOS, while Norton\'s Mac version is more feature-complete than it used to be.' },
            { q: 'Can I use either on my phone?', a: 'Yes. Both have iOS and Android apps with real-time scanning, safe browsing, and anti-phishing features. Bitdefender Mobile Security is $14.99/yr standalone; Norton includes mobile in most bundle plans.' },
          ].map((f, i) => (
      ...
      <div className="my-8">
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          ...
          <Link href="/compare/surfshark-vs-nordvpn" className="block border border-gray-700 rounded-xl p-5 hover:border-emerald-700 transition-colors group">
            ...
            <p className="text-xs text-gray-400 mt-1">Unlimited devices vs top speed</p>
          </Link>
        </div>
      </div>
